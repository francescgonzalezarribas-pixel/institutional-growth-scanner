...
# ===================== GEMINI (mejorado) =====================
def ask_gemini(prompt, max_tokens=700):
        ...
        payload = {
            "contents": [{
                "parts": [{"text": (
                    ...
                    "No cortes las frases. Sé concreto.\n\n" + prompt
                )}]
            }],
            "generationConfig": {
...
def calculate_value_index(raw):
    ...
    if (hist.empty or len(hist) < 20) and is_crypto:
        ...
        outlook = ask_gemini(
            f"Activo: {cg['name']} ({cg['symbol']}). Precio: ${cg['price']}. "
            f"Cambio 24h: {cg['d1']:.1f}%. Cambio 7d: {cg['d7']:.1f}%. "
            f"Desde máximos: {cg['ath_change']:.1f}%. Fear & Greed: {fg}. Score: {score}/100. "
            f"Dame una perspectiva clara de 3-4 frases sobre qué esperar."
    ...
    outlook = ask_gemini(
        f"Activo: {name} ({ticker}). Precio: {price:.4f}. RSI: {rsi:.1f}. "
        f"Distancia al máximo 52 semanas: {dist_52:.1f}%. Score: {score}/100 ({label}). "
        f"Dame una perspectiva clara de 3-4 frases sobre qué esperar a corto plazo."
    ...
    try:
        ...
        for s in signals:
            explicacion = ask_gemini(
                f"Señal de trading: {s['nombre']} ({s['ticker']}). "
                f"Entrada {s['entry']}, Stop {s['stop']}, TP1 {s['tp1']}, R/R {s['rr']}x. "
                f"RSI {s['rsi']:.1f}, Volumen {s['vol_rel']:.1f}x. Motivos: {', '.join(s['motivos'])}. "
                f"Explica en 3 frases por qué es interesante y cuál es el riesgo principal."
            )
            ...
            if explicacion:
                caption += f"\n\n**Análisis IA:**\n{explicacion}"
            # Si el caption es muy largo, enviamos texto aparte
            if len(caption) > 1000:
                bot.send_photo(message.chat.id, create_signal_chart(s))
...
def send_analiza(message, raw):
    ...
    try:
        ...
        prompt = (
            f"Análisis del activo {data['name']} ({data['ticker']}).\n"
            f"Precio: {ps} (24h: {data['d1']:+.2f}%)\n"
            f"Score de valor: {data['score']}/100 — {data['label']}\n\n"
            f"Estructura la respuesta así:\n"
            f"1) Situación actual\n2) Qué esperar a corto plazo\n3) Nivel de riesgo\n4) Conclusión clara"
        )
        analisis = ask_gemini(prompt, max_tokens=900)
...
def send_valor(m, raw):
    ...
    try:
        ...
        # Caption corto + texto IA aparte si es largo
        ...
        bot.send_photo(m.chat.id, img, caption=cap)
        # Perspectiva IA en mensaje separado (evita cortes)
        bot.send_message(m.chat.id, f"**Qué esperar:**\n{data['outlook']}", reply_markup=get_kb())
    except Exception as e:
...
def noticias(m):
    ...
    if heads and GEMINI_API_KEY:
        resumen = ask_gemini(
            "Resume estas noticias financieras en 4-5 puntos claros y al final indica el sesgo general del mercado (alcista, bajista o neutral):\n"
            + "\n".join(heads[:6]),
            max_tokens=800
        )
        if resumen:
...
if __name__ == "__main__":
    print("Bot + Gemini mejorado iniciado")
