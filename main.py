...
def get_realtime_btc():
  try:
    r = requests.get(
        "https://api.binance.com/api/v3/ticker/24hr?symbol=BTCUSDT", timeout=6
    )
    data = r.json()
    return {
...
def get_label(score: float) -> str:
  if score >= 75:
    return "BARATO — OPORTUNIDAD"
  if score >= 60:
    return "NEUTRAL — ACUMULACIÓN"
  if score >= 45:
    return "NEUTRAL"
  if score >= 30:
    return "CARO — PRECAUCIÓN"
...
def calculate_value_index(ticker: str) -> dict:
  ...
  is_crypto = (
      any(x in ticker for x in ["BTC", "ETH", "SOL", "BNB"]) or "-USD" in ticker
  )
  ...
  try:
    benchmark = yf.Ticker("BTC-USD" if is_crypto else "SPY").history(
        period="3mo"
    )["Close"]
    rel_str = (
        (price / float(close.iloc[-21]))
        - (float(benchmark.iloc[-1]) / float(benchmark.iloc[-21]))
    ) * 100
  except Exception:
    rel_str = 0.0
  ...
  comps_raw = [
      ...
      (
          ...
          if macd_data["cross_up"]
              ...
              "Cruce ↓"
              if macd_data["cross_down"]
              else f"{macd_data['hist']:+.1f}"
          ),
          score_macd(macd_data),
      ),
...
def create_gauge_image(
    score: int, title: str, label: str, components: list
) -> io.BytesIO:
  ...
  ax1.plot(
      [0, 0.95 * math.cos(angle_rad)],
      [0, 0.95 * math.sin(angle_rad)],
      color="white",
      linewidth=5.5,
  ...
  for i, (name, value, sc) in enumerate(reversed(components)):
    ...
    ax2.text(
        0.1, y, name, ha="left", va="center", fontsize=10, color="#e6edf3"
    )
    ax2.text(
        2.6, y, value, ha="right", va="center", fontsize=9, color="#8b949e"
    )
    ax2.text(
        8.5,
        y,
...
def analisis_btc_profundo():
  ...
  d20 = (
      (price - float(close.iloc[-21])) / float(close.iloc[-21]) * 100
      if len(close) > 20
      else 0
  )
...
def send_btc_profundo(message):
  ...
  try:
    ...
    lines = [
        ...
        "",
        (
            f"**S/R**  R1: {d['r1']:,.0f}  |  Pivot: {d['pivot']:,.0f}  |  S1:"
            f" {d['s1']:,.0f}"
        ),
    ]
    if d["niveles"]:
    ...
    if d["long_short"]:
      ls = d["long_short"]
      lines.append(
          f"**Long/Short:** {ls['ratio']} ({ls['long_pct']}% /"
          f" {ls['short_pct']}%)"
      )
    if d["dominancia"]:
      dom = d["dominancia"]
      lines.append(
          f"**Dominancia BTC:** {dom['btc_dom']}% | ETH: {dom['eth_dom']}%"
      )
...
# ===================== MENÚS Y BOTONES INTERACTIVOS =====================
def get_main_keyboard():
  kb = InlineKeyboardMarkup(row_width=2)
  kb.add(
      InlineKeyboardButton("💎 Índice Valor", callback_data="valor_menu"),
      InlineKeyboardButton("₿ BTC Profundo", callback_data="btc_profundo"),
      InlineKeyboardButton("📰 Noticias", callback_data="noticias"),
      InlineKeyboardButton("🔍 Investigar", callback_data="investigar_menu"),
      InlineKeyboardButton("📊 Menú", callback_data="menu"),
  )
  return kb


def get_quick_tickers_keyboard(prefix: str):
  kb = InlineKeyboardMarkup(row_width=2)
  kb.add(
      InlineKeyboardButton("NVIDIA (NVDA)", callback_data=f"{prefix}_NVDA"),
      InlineKeyboardButton("Tesla (TSLA)", callback_data=f"{prefix}_TSLA"),
      InlineKeyboardButton("Bitcoin (BTC)", callback_data=f"{prefix}_BTC"),
      InlineKeyboardButton("Apple (AAPL)", callback_data=f"{prefix}_AAPL"),
      InlineKeyboardButton(
          "✍️ Escribir manualmente", callback_data=f"{prefix}_custom"
      ),
  )
  return kb
...
def send_welcome(message):
  if not is_authorized(message.from_user.id):
    return
  text = (
      "🤖 **Bot Financiero Interactivo**\n\n"
      "Usa los botones del menú o los comandos directos:\n"
      "• `/valor TICKER` → Medidor de valor + gráfico + noticias\n"
      "• `/btc` → Análisis técnico y métricas de Bitcoin\n"
      "• `/noticias` → Noticias de mercado\n"
      "• `/investigar TICKER` → Datos fundamentales del activo"
  )
...
def handle_query(call):
  ...
  # Menú principal de comandos inmediatos
  ...
  # Desplegables interactivos para Índice Valor
  elif call.data == "valor_menu":
    bot.answer_callback_query(call.id)
    bot.send_message(
        call.message.chat.id,
        "💎 **Selecciona un activo o escribe el que quieras:**",
        reply_markup=get_quick_tickers_keyboard("qv"),
    )

  elif call.data.startswith("qv_"):
    bot.answer_callback_query(call.id)
    ticker = call.data.replace("qv_", "")
    if ticker == "custom":
      msg = bot.send_message(
          call.message.chat.id,
          "💎 **Escribe el ticker que deseas analizar:**\n(Ejemplo: `AMZN`,"
          " `MSFT`, `SOL`)",
          reply_markup=ForceReply(selective=False),
      )
      bot.register_next_step_handler(msg, process_valor_step)
    else:
      send_valor(call.message, ticker)

  # Desplegables interactivos para Investigar
  elif call.data == "investigar_menu":
    bot.answer_callback_query(call.id)
    bot.send_message(
        call.message.chat.id,
        "🔍 **Selecciona un activo o escribe el que quieras investigar:**",
        reply_markup=get_quick_tickers_keyboard("qi"),
    )

  elif call.data.startswith("qi_"):
    bot.answer_callback_query(call.id)
    ticker = call.data.replace("qi_", "")
    if ticker == "custom":
      msg = bot.send_message(
          call.message.chat.id,
          "🔍 **Escribe el ticker que deseas investigar:**\n(Ejemplo: `AMD`,"
          " `META`, `GOOGL`)",
          reply_markup=ForceReply(selective=False),
      )
      bot.register_next_step_handler(msg, process_investigar_step)
    else:
      do_investigation(call.message, ticker)


# ===================== LÓGICA PASO A PASO =====================
def process_valor_step(message):
  if not is_authorized(message.from_user.id):
    return
  ...
  if text.startswith("/VALOR"):
    ...
    ticker = text
  send_valor(message, ticker)


def process_investigar_step(message):
  if not is_authorized(message.from_user.id):
    return
  text = message.text.strip().upper()
  if text.startswith("/INVESTIGAR"):
    ...
    ticker = text
  do_investigation(message, ticker)
...
def cmd_valor(message):
  ...
  if len(args) < 2:
    bot.reply_to(
        message,
        "⚠️ Especifica un ticker. Ejemplo: `/valor TSLA`",
...
def send_valor(message, ticker):
  ...
  try:
    ...
    img = create_gauge_image(
        data["score"], title, data["label"], data["components"]
    )
    ...
    bot.reply_to(
        message, f"⚠️ Error con {ticker}: {e}", reply_markup=get_main_keyboard()
    )
...
def send_noticias(message):
  ...
  text = "📰 **NOTICIAS DE IMPACTO**\n\n" + "\n".join(
      headlines[:6] or ["• Sin noticias disponibles ahora"]
  )
...
def send_investigacion(message):
  ...
  if len(args) < 2:
    ...
    return
  do_investigation(message, args[1].upper())


def do_investigation(message, ticker):
  bot.send_chat_action(message.chat.id, "typing")
  try:
    stock = yf.Ticker(ticker)
    info = stock.info
    ...
    msg = (
        f"🔍 **{name} ({ticker})**\n\n"
        f"• Precio: **{price}**\n• P/E: {pe}\n• Market Cap: {mcap_str}\n"
        f"• Sector: {info.get('sector', 'N/D')}\n\n_Escribe /valor {ticker} para"
        " ver su medidor visual._"
    )
...
def handle_free(message):
  ...
  if message.text and not message.text.startswith("/"):
    bot.reply_to(
        message,
        "Usa el menú inferior para navegar o los comandos directos (`/valor`,"
        " `/btc`, `/noticias`).",
        reply_markup=get_main_keyboard(),
    )
...
if __name__ == "__main__":
  print("🤖 Bot Financiero con Botones Interactivos iniciado...")
