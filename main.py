...
def fetch_signal_data(ticker):
    try:
        ...
        return {
            ...
            "hist": hist, "sobre_ema20": price > ema20, "tendencia": ema20 > ema50
        }
    except:
        return None
...
def get_top_signals(stocks, n=4, min_score=6):
    ...
    for t in stocks:
        ...
        if not d or d["rsi"] > 70 or d["vol_rel"] < 0.65:
            continue
        ...
        if d["rsi"] < 32:
            score += 5
            motivos.append(f"RSI {d['rsi']:.0f} sobreventa")
        elif d["rsi"] < 42:
            score += 4
            motivos.append(f"RSI {d['rsi']:.0f} zona entrada")
        elif d["rsi"] < 52:
            score += 2
            motivos.append(f"RSI {d['rsi']:.0f}")
        ...
        if d["vol_rel"] >= 1.7:
            score += 3
            motivos.append(f"Vol {d['vol_rel']:.1f}x")
        elif d["vol_rel"] >= 1.15:
            score += 1
        ...
        cands.append({
            **d,
            ...
            "score": score,
            "motivos": motivos,
            "entry": round(entry, 2 if entry > 10 else 4),
            "stop": stop,
            "tp1": tp1,
            "tp2": tp2,
...
def send_signals(message, region="US"):
    ...
    try:
        ...
        if not signals:
            bot.send_message(
                message.chat.id,
                f"No hay señales claras en {region} ahora mismo.\nPrueba más tarde o usa `/valor TICKER`.",
                reply_markup=get_kb()
            )
            return
        ...
        bot.send_message(message.chat.id, f"⚠️ Error generando señales: {e}", reply_markup=get_kb())
...
def start(m):
    ...
    bot.reply_to(m,
        ...
        "• `/btc` → Bitcoin\n\n"
        "_Ejemplos: /valor BTC  /valor ORO  /valor TSLA  /valor SAN.MC_",
        reply_markup=get_kb())
...
def cmd_valor(m):
    ...
    args = m.text.split()
    if len(args) < 2:
        bot.reply_to(m, "⚠️ Usa: `/valor TSLA` o `/valor ORO` o `/valor BTC`")
        return
    send_valor(m, args[1])
    ...
    for url in ["https://www.expansion.com/rss/mercados.xml", "https://cincodias.elpais.com/rss/cincodias/portada.xml"]:
        try:
            for e in feedparser.parse(url).entries[:3]:
                if e.title not in heads:
                    heads.append(f"• {e.title}")
        except:
            pass
...
if __name__ == "__main__":
    print("Bot señales + gráficos (tiempo real) iniciado")
